document.addEventListener('DOMContentLoaded', () => {
    // A variável 'alunoId' agora vem diretamente do PHP.
    // O seu código pode acessá-la sem precisar de getElementById
    const calendarPlaceholder = document.getElementById('calendar-placeholder');
    
    // NOTA: Removida a linha `const alunoIdInput = document.getElementById('calendario_id');`
    // e o `if` que a verifica, pois a variável 'alunoId' será global e estará disponível.
    if (!calendarPlaceholder || typeof alunoId === 'undefined') {
        console.error('Um dos elementos necessários (calendar-placeholder ou aluno_id) não foi encontrado.');
        return;
    }

    // A variável alunoId é definida diretamente no seu HTML pelo PHP, por isso não precisa ser buscada aqui
    ...
    function loadEvents() {
        fetch(`get_eventos.php?aluno_id=${alunoId}&month=${currentMonth + 1}&year=${currentYear}`)
            .then(res => res.json())
            .then(data => {
